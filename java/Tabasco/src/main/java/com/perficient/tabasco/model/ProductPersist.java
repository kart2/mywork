...
public class ProductPersist extends Model {
  ...
  public static HashMap<String,Object> page(int page, int recordsPerPage) {
    ArrayList<Product> products = new ArrayList<Product>();
 
    Connection dbConnection = null;
    PreparedStatement preparedStatement = null;
 
    String countSQL = "SELECT COUNT(*) AS total_records from product";
    String selectSQL = "SELECT product_id, name, item, model, description, url, price FROM product ORDER BY product_id OFFSET 32 LIMIT 32";
 
    try {
      dbConnection = getDBConnection();
      preparedStatement = dbConnection.prepareStatement(countSQL);

      // execute count SQL stetement
